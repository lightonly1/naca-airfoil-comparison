...
│   ├── geometry\_engine.py     <- Core mathematical and extrusion functions

│   ├── visualization.py       <- Automation plotting engine

│   └── run\_pipeline.py        <- Core processing runner engine

├── notebooks/

│   └── aerodynamic\_exploration.ipynb <- Historical exploratory R\&D workspace

└── tests/

&#x20;   └── test\_geometry.py       <- Automated mathematical boundary test maps
